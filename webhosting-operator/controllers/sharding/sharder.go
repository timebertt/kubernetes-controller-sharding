...
func (r *shardingReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if currentShard != "" {
		currentShardState = r.ActualStateOfWorld.GetShardState(currentShard)
...
func (r *shardingReconciler) SetupWithManager(mgr manager.Manager) error {
	...
	return builder.ControllerManagedBy(mgr).
		Named("sharder").
		For(r.metaObject, builder.WithPredicates(sharderPredicate)).
		// TODO enqueue objects on lease state change
...
// TODO only enqueue object if unassigned
var sharderPredicate = predicate.Funcs{}
