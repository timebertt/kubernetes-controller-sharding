...
func (o *Operation) ResyncControllerRing(ctx context.Context, log logr.Logger) error {
	var (
		...
		queue = make(chan *workItem, o.Concurrency)
	)

	// compile all objects that need to be moved or drained, add them to the queue
	// the buffer limit of the queue applies backpressure on the work generator (throttling list paging as needed)
	wg.Start(func() {
		o.compileWorkForControllerRing(ctx, log, queue, errs)
		close(queue)
	})

	// read from the queue and perform drains and movements with the configured concurrency
	for i := 0; i < o.Concurrency; i++ {
		wg.Start(func() {
			for o.processNextWorkItem(ctx, log, queue, errs) {
			}
		})
	}
...
func (o *Operation) compileWorkForControllerRing(ctx context.Context, log logr.Logger, work chan<- *workItem, errs chan<- error) {
	// resync all ring resources
	for _, ringResource := range o.ControllerRing.Spec.Resources {
		errs <- o.compileWorkForResource(ctx, log, ringResource.GroupResource, false, work)

		for _, controlledResource := range ringResource.ControlledResources {
			errs <- o.compileWorkForResource(ctx, log, controlledResource, true, work)
		}
	}
}

func (o *Operation) compileWorkForResource(
	ctx context.Context,
	log logr.Logger,
	gr metav1.GroupResource,
	controlled bool,
